...
func (s *orderServiceImpl) CreateOrder(ctx context.Context, userID string, req dto.CreateOrderRequestDTO) (*dto.OrderResponseDTO, error) {
	...
	// 1. Merge duplicates & validate quantities while preserving input order
	mergedQuantities := make(map[string]int)
	var orderedBookIDs []string

	for _, item := range req.Items {
		if item.Quantity <= 0 {
			return nil, fmt.Errorf("invalid quantity %d for book %s", item.Quantity, item.BookID)
		}
	...
	// 2. Fetch current prices & verify availability preserving item order
	...
	for _, bookID := range orderedBookIDs {
		...
		if !book.CanFulfill(qty) {
			return nil, fmt.Errorf("insufficient stock for '%s': requested %d, available %d", book.Title, qty, book.Stock)
		}
